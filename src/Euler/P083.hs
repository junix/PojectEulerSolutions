...
initm xs = newListArray (0, lastPos) xs' :: ST s (STArray s Integer (Integer,Integer))
   where lastPos = toInteger $ length xs - 1
         xs'     = map (\x -> (10^10,x)) xs

m2v l x y = y*l + x

nei l x y = filter validPos [(x-1,y), (x+1,y), (x,y-1), (x,y+1)]
    where inR v = v >= 0 && v < l
          validPos (a,b) = inR a && inR b

initLT :: STArray s Integer (Integer,Integer)
...
update l m = go m 0 0 False
   where go :: STArray s Integer (Integer,Integer)
            -> Integer
            -> Integer
            -> Bool
            -> ST s (STArray s Integer (Integer, Integer))
